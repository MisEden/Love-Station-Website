...
let navList = {
    "title": ["棧點介紹", "愛心棧作業要點", "入住暨退房須知"],
    "url": ["stack.html", "rule.html", "notice.html"]
};



function showMenu(currentPage_fileName = "") {
    var $menu = $("<ul class=\"navbar-nav mr-auto\"></ul>");

    $.each(navList.title, function(i, val) {
        var $item = $("<li class=\"nav-item\"></li>");

        if (currentPage_fileName === navList.url[i]) {
            var $title = $("<a class=\"nav-link active\" href=\"" + navList.url[i] + "\"></a>").html(val);
        } else {
            var $title = $("<a class=\"nav-link\" href=\"" + navList.url[i] + "\"></a>").html(val);
        }
