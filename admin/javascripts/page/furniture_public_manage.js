...
function save_edit(precedence, id){
    ...
    if(furnitureNameBeforeEdit != document.getElementById("input_furnitureName_" + precedence).value){
        ...
        fetch(API_url + '/v1/api/admins/furniture/public/rename',{
            method: 'PUT', 
...
function insert(){
    ...
    fetch(API_url + '/v1/api/admins/furniture/public',{
        method: 'POST', 
        body: JSON.stringify(Jsonarr), 
        headers: new Headers({
            'Content-Type': 'application/json',
...
function changeSerial(){
    ...
    fetch(API_url + '/v1/api/admins/furniture/public/exchange',{
        method: 'PUT', 
        body: JSON.stringify(Jsonarr), 
        headers: new Headers({
            'Content-Type': 'application/json',
...
function confirmDelete(id, name){
    var yes = confirm('刪除家具會連帶移除所有棧點內「配置此家具的紀錄」&「針對此家具的入住確認」\n你確定要將「' + name + '」刪除嗎？');
    ...
    if (yes) {
        ...
        fetch(API_url + '/v1/api/admins/furniture/public/' + id,{
            method: 'DELETE', 
            body: JSON.stringify(Jsonarr), 
            headers: new Headers({
                'Content-Type': 'application/json',
