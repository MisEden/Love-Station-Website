...
const roomId = getParameterByName('room', "house.html?page=0");
var houseName = "";
var roomNumber = 0;
...
window.onload = function() {
    var path = ["首頁", "棧點管理", "現有棧點", "棧點房間", "棧點房間家具"]; 
    showBreadcrumb(path);
...
function getRoomName(){
    fetch( API_url + '/v1/api/rooms/' + roomId + '/name', {
        method: 'GET',
        headers: new Headers({
...
function getExistFurnitrue(){
    fetch( API_url + '/v1/api/checkin/room/' + roomId + '/furniture/private', {
        method: 'GET',
        headers: new Headers({
            'Content-Type': 'application/json'
        })
...
function loadData(){
    ...
    .then(function checkStatus(response) {
        ...
        document.getElementById("roomNumber_list").innerHTML = roomNumber;
...
function isExistFurniture(furnitureName){
    for(var i = 0; i < existFurniture.length; i++){
        if(existFurniture[i].privateFurnitureName === furnitureName){
            return true
        }
    }
...
function save(precedence){
    ...
    if(beforeSave != document.getElementById("checkExist_" + precedence).checked){
        ...
        fetch(API_url + '/v1/api/admins/furniture/private/status',{
            method: 'PUT', 
