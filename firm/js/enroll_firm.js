...
window.onload = function () {
  ...
  const defaultLiffId = "1655081006-kZM9oGZ7";
  let myLiffId = "";
  ...
  fetch(API_url + '/v1/api/firms/names/all', {
    method: 'GET',
    headers: new Headers({
    ...
    if (response.status == 200) {
      ...
      var where = 'enroll_firm';
      fetch_error(response, where);
    }
  }).then((jsonData) => {
    getName = jsonData;
    console.log(jsonData)
    var select = document.getElementById("inputfirmName");
    for (var i = 0; i < getName.length; i++) {
      select.innerHTML += '<option >' + getName[i].firmName + '</option>';
      firmList.push({
        firmId: getName[i].firmId,
        firmName: getName[i].firmName
      });
    }
  }).catch((err) => {
...
function enroll() {
  ...
  var firmName = document.getElementById("inputfirmName").value;
  ...
  for (var i = 0; i < getName.length; i++) {
    if (firmName == firmList[i].firmName) {
      var firmId = firmList[i].firmId
    }
  }
  ...
  if (account == '' || password == '' || chineseName == '' || email == '' || birthday == '' || vercode == '' || identityCard == '' || gender == '' || address == '' || phone == '' || cellphone == '' || agreePersonalInformation == '' || document.getElementById("inputAddress").value == '') {
    ...
    $.ajax({
      type: "POST",
      url: API_url + "/v1/api/firm-employees",
      async: false,
      ...
      success: function () {
        ...
        setTimeout(function () {
          window.location.href = 'https://liff.line.me/1655081006-dOxOeDm2';
        }, 3000);
      }
      ,
