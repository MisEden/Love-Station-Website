var getReferralDepId;
var keyWord = "";
var check = 0;

window.onload = function() {
    const defaultLiffId = "1655081006-rRBXdEgA";
    let myLiffId = "";
...
// 轉介單位列表
fetch(API_url + '/v1/api/referrals', {
        method: 'GET',
        headers: new Headers({
...
// 轉介單位職稱
var getReferralTypeId;
fetch(API_url + '/v1/api/referral-titles', {
        method: 'GET',
        headers: new Headers({
            'Content-Type': 'application/json'
        }),
    ...
    .then(function checkStatus(response) {
        ...
        var referral_info = jsonData;
        getReferralTypeId = referral_info;
        var select = document.getElementById("ReferralType");

        for (var i = 0; i < referral_info.length; i++) {

            select.innerHTML += '<option> ' + referral_info[i].name + '</option>';
        }
...
myFile.addEventListener('change', function(e) {
    ...
    if (upload.size > 50000000) {
        ...
        watermark([upload])
            .image(watermark.text.center('僅供伊甸使用', '160px serif', 'red', 0.5))
            .then(img => {
                img.id = "image1";
                img.width = 300;
...
function enroll() {
    ...
    check = 0;
    var referralDepindex = document.getElementById("ReferralName").selectedIndex;
    var referralTypeindex = document.getElementById("ReferralType").selectedIndex;

    var referralId = getReferralDepId[referralDepindex].id;
    document.getElementById("referralId").value = referralId;
    var lineId = lineUserId;
    document.getElementById("lineId").value = lineId;
    var referralTitleId = getReferralTypeId[referralTypeindex].id;
    document.getElementById("referralTitleId").value = referralTitleId;
    ...
    if (document.getElementById("agree").value == "1") {
        formData.set('agreePersonalInformation', true);
    }
    ...
    if (inputPassword1 != inputPassword2) {
        ...
        formData.append('password', inputPassword1);
    }
    ...
    */
    ...
    if (check != 1) {
        $.ajax({
            url: API_url + "/v1/api/referral-employees",
            type: 'POST',
            headers: {},
            ...
            success: function(data, textStatus, request) {
                ...
                setTimeout(function() {
                    window.open('https://liff.line.me/1655081006-dOxOeDm2', '_self');
                }, 3000);
            },
...
function loadReferralList() {
    var select = document.getElementById("ReferralName");
    select.innerHTML = "";

    for (var i = 0; i < getReferralDepId.length; i++) {
        if (getReferralDepId[i].hospitalChineseName.indexOf(keyWord) != -1) {
            select.innerHTML += '<option> ' + getReferralDepId[i].hospitalChineseName + '</option>';
        }
    }
}

function searchReferral() {
    keyWord = document.getElementById("input_searchReferral").value;
    loadReferralList()
}
